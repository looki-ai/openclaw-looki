...
export async function runForwardSelectionFlow(deps: {
  ...
  if (detected.length === 0) {
    await prompter.note(t("plugin.none"), t("plugin.title"));
    return [];
  }
  ...
  const disabledLine =
    disabledTargets.length > 0
      ...
      : "";
  await prompter.note([detectedLine, disabledLine].filter(Boolean).join("\n"), t("plugin.title"));
  ...
  while (true) {
    await prompter.note(t("listControls.body"), t("listControls.title"));
