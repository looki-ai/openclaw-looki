...
async function executeLookiTaskAction(
  ...
  switch (action) {
    ...
    case "task_notification": {
      ...
      return formatToolResult(
        action,
        await lookiApiPost(cfg, `tasks/${encodeURIComponent(taskId)}/message_notice`, {
          enabled,
        }),
      );
    }
...
const LOOKI_TASK_TOOL_PARAMETERS = {
  type: "object",
  ...
  properties: {
    ...
    enabled: {
      type: "boolean",
      description:
        "For task_notification: true to enable openclaw message_channel forwarding, false to disable.",
    },
  },
  required: ["action"],
} as const;

function makeLookiTaskExecute(getConfig: () => OpenClawConfig, logger?: ToolLogger) {
...
export const LOOKI_TASK_TOOL = {
  name: "looki_task",
  label: "Looki Task",
  description:
    "List the user's Looki tasks and toggle the openclaw message_channel switch per task, using the configured channels.openclaw-looki baseUrl/apiKey.",
  parameters: LOOKI_TASK_TOOL_PARAMETERS,
  makeExecute: makeLookiTaskExecute,
} as const;
