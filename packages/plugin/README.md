...
- Long-polls Looki's `/message_channel/get_updates` and delivers events to your OpenClaw agent
...
- Ships two tools + matching skills so the agent can read Looki directly:
  ...
  - `looki_task` / `looki-task` — list Looki tasks and toggle per-task
    openclaw message_channel forwarding
...
- `sessionKey` is written by setup when you choose an existing OpenClaw
  session. The plugin derives direct/group routing from that session key.
...
[`src/tools/task-tool.ts`](./src/tools/task-tool.ts)
